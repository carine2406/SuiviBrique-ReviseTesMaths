...
export default function TeacherDashboard() {
    ...
    const [newEleveData, setNewEleveData] = useState({ nom: '', prenom: '', classe_id: '' })
    ...
    const handleAddEleve = async () => {
        if (!newEleveData.nom || !newEleveData.prenom || !newEleveData.classe_id) return
        try {
    ...
    const handleDeleteEleve = async (id) => {
        if (!confirm("Êtes-vous sûr de vouloir supprimer cet élève ? Toutes ses données (notes, planning) seront définitivement effacées.")) return
        try {
            const { error } = await supabase
                .from('eleves')
                .delete()
    ...
    const fetchEleveDetails = async (eleve) => {
        ...
        try {
            ...
            setElevePlanning(planningData?.indicateurs ? [planningData.indicateurs] : [])
        ...
        <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            {teacher.code_professeur && (
                                <div className="hidden md:flex flex-col items-end px-4 border-l border-slate-800">
                                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-tighter">Code Excel</p>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] flex items-center gap-6">
                        ...
                        <div>
                            <p className="text-slate-400 text-sm font-bold uppercase tracking-tight">Dernières Notes</p>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-3 bg-slate-950 border border-slate-800 rounded-2xl px-4 py-2">
                                ...
                                >
                                    ...
                                    {classes.map(c => (
                                        <option key={c.id} value={c.id} className="bg-slate-900">{c.nom}</option>
                                    ))}
                                </select>
                            </div>
                            <button
                                ...
                                {filteredEleves.map((eleve) => (
                                    <tr key={eleve.id} className="group hover:bg-slate-800/20 transition-all">
                                        ...
                                        <td className="px-6 py-6 text-center">
                                            <span className="text-slate-400 font-bold">{eleve.classes?.nom}</span>
                                        </td>
                                        <td className="px-6 py-6 text-center">
            ...
            {showEditModal && eleveToEdit && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
                    <div className="bg-slate-900 w-full max-w-md rounded-[2.5rem] p-8 sm:p-10 border border-slate-800 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="space-y-6">
                            <h2 className="text-2xl font-bold text-white">Modifier la classe</h2>
                            <p className="text-slate-400 text-sm">
                                Changement de classe pour <strong>{eleveToEdit.prenom} {eleveToEdit.nom}</strong>
                            </p>
                            <div className="space-y-4">
